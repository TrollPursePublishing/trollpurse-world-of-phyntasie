...
namespace WebApi.Backend.Admin
{
    ...
    public class ApplicationUserManager : UserManager<ApplicationUser>
    {
        ...
        public static ApplicationUserManager Create(IdentityFactoryOptions<ApplicationUserManager> options, IOwinContext context)
        {
            ...
            if (manager.Users.FirstOrDefault(u => u.UserName.Equals("aqgAdmin")) == null)
            {
                var result = manager.Create(new ApplicationUser(Guid.Empty)
                {
                    UserName = "aqgAdmin",
                    EmailConfirmed = true,
                    Email = "non@non.org",
                ...
                if (result.Succeeded)
                {
                    var user = manager.Users.Single(u => u.UserName.Equals("aqgAdmin"));

                    try
                    {
                        manager.AddClaim(user.Id, new Claim(ClaimTypes.Role, "Admin"));
                    }
                    catch (Exception)
                    { }
                }
