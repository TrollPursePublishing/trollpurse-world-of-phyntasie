...
function load(slotName = "autosave") {
  ...
  const GameContextObj = localStorage.getItem(contextKey)
    ? gameContext(JSON.parse(localStorage.getItem(contextKey)))
    : gameContext(defaultGameContext());

  const GameContext = {
    ...GameContextObj,
    world: world(GameContextObj.world),
    relics: GameContextObj.relics.map(r => relic(r)),
  };

  const PlayerObj = localStorage.getItem(playerKey)
  ...
  const Player = player({
    ...PlayerObj,
    spells: PlayerObj.map(spell => spell({ ...allSpells[spell.name], name: spell.name })),
...
function getBuyList() {
  const { Player } = load();
  return Player.currentLocation.market
